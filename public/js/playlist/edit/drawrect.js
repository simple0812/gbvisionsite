...
canvasEditor.subscribe('moving', function(e, x, y) {
    drawInfo.operRect.left = drawInfo.operRect.parent? x - moveInfo.x - drawInfo.operRect.parent.left : x - moveInfo.x;
    drawInfo.operRect.top =  drawInfo.operRect.parent? y - moveInfo.y - drawInfo.operRect.parent.top : y - moveInfo.y;
...
canvasEditor.subscribe('copy', function(e, x, y) {
    ...
    if(!drawInfo.copy)
        ...
        drawInfo.operRect.left = drawInfo.operRect.parent? drawInfo.operRect.left + drawInfo.operRect.parent.left : drawInfo.operRect.left;
        drawInfo.operRect.top =  drawInfo.operRect.parent? drawInfo.operRect.top + drawInfo.operRect.parent.top : drawInfo.operRect.top;
...
cvsarea.onmousedown = function(e) {
    ...
    if(drawInfo.action == 'translate')  return canvasEditor.publish('pretranslate',[e.clientX, e.clientY]);

    if(drawInfo.action == 'selected') {
        if(! checkPointIsInRect(x, y, drawInfo.operRect) && src.className != 'dot' && src.className != 'movediv')
            canvasEditor.publish('unselect');
        else sRect = null;
    }

    if(sRect) canvasEditor.publish('select', [sRect]);
    else if(src.className == 'topcanvas') canvasEditor.publish('prestroke',[x, y, src]);
...
window.onmousemove = function(e) {
    ...
    if(e.altKey && drawInfo.action == 'move') canvasEditor.publish('copy',[x, y]);
    else if(drawInfo.action == 'selected' && src.className == 'movediv') canvasEditor.publish('premove',[x, y]);
    else if(drawInfo.action == 'selected' && src.className == 'dot') canvasEditor.publish('preresize',[src]);
    else if(drawInfo.action == 'translate') canvasEditor.publish('translating',[e.clientX, e.clientY]);
    else if(drawInfo.action == 'stroke') canvasEditor.publish('stroking',[x, y]);
    else if (drawInfo.action == 'resize') canvasEditor.publish('resizing',[drawInfo.operRect, x, y]);
...
window.onmouseup = function(e) {
	...
	if(e.button != 0 || !drawInfo.operRect || !drawInfo.leftBtnDown
        || !drawInfo.action || drawInfo.action == 'selected') return drawInfo.leftBtnDown = false;

    if( (drawInfo.action == 'move' &&
        (drawInfo.operRect.left ==drawInfo.origRect.left && drawInfo.operRect.top == drawInfo.origRect.top)) ) {
...
function preStroke(x, y, ele) {
    ...
    if(drawInfo.operRect.constructor == ImageRect || drawInfo.operRect.constructor.superClass == ImageRect) {
        var obj = $('#toolbar .btn-primary img').length == 0 ?  $('#toolbar .btn-primary') : $('#toolbar .btn-primary img');
        var img= new Image();
        img.onload = function() {
            var w = img.width;
            var h = img.height;

            drawInfo.operRect.source = new ImageInfo(img.src.replace(/http:\/\/[^\/]+(:\d{2,8})?\//, '/'), w, h)
            drawInfo.operRect.url = img.src.replace(/http:\/\/[^\/]+(:\d{2,8})?\//, '/');

            if(!_.find(IMAGES, function(each) { return each.src.trimUrlPath() == img.src.replace(/http:\/\/[^\/]+(:\d{2,8})?\//, '/')}))
                IMAGES.push({src:img.src.replace(/http:\/\/[^\/]+(:\d{2,8})?\//, '/'), image:img});
...
function stroking(rect) {
    ...
    tempCtx.strokeRect(rect.parent? rect.left + rect.parent.left : rect.left,
...
function stroke(rect) {
    if(!rect.bottom || ! rect.right || Math.abs(rect.width)  < 5 ||  Math.abs(rect.height)  < 5 ) {
        drawInfo.leftBtnDown = false;
        return;// alert('矩形过小');
    }
...
function removeRect(tempRect) {
    if(drawInfo.operRect && drawInfo.operRect.parent) {
        ...
        if(tempRect.constructor == MediaZone) {//删除媒体资源
            delMediaSourceById(tempRect.playlist);
...
function render() {
    ...
    cvs.width += 0;
    $('#compnentindex').empty();
    for(var i = 0, len = relativePlaylist.children[viewIndex].children.length; i < len ; i++) {
    ...
    if(!getSelectedRect()) {
        ...
        xCtx.putImageData(ctx.getImageData(0, 0, WIDTH * RATIO, HEIGHT * RATIO), 0, 0);
        showCurrentArea();
    }

    if(currScene.background.image) {
        $('#canvas').css('background','url("'+currScene.background.image+'") ').css('backgroundSize','100% 100%');
        $('#leftside canvas').eq(viewIndex).css('background','url("'+currScene.background.image+'") ').css('backgroundSize','100% 100%');
    }
    else if(currScene.background.color) {
        $('#canvas').css('background', hexToRgb(currScene.background.color));
        $('#leftside canvas').eq(viewIndex).css('background', hexToRgb(currScene.background.color));
    }
...
function showCurrentArea() {
    ...
    //if(w > wImg) w = wImg;
    //if(h > hImg) h = hImg;
    $(div).css({width:w, height:h, border:'1px solid red', position:'absolute', top:t, left:l, opacity:0.3, background:'yellow'});
...
function showSelectedRect(rect) {
    ...
    var cursors = ['nw-resize', 'n-resize', 'ne-resize', 'w-resize',
...
function showMediaPlaylist(playlist) {
    $('#mediaElements').empty();
    if(!playlist || playlist.children.length == 0) return;

    for(var i = 0, len = playlist.children.length; i < len; i++) {
        var media = playlist.children[i];
        var type = media.type;
        var src = media.url;
        switch (type) {
            ...
            case 'ppt': src = '/images/ppt300.png'; break;
            default : src = media.url; break;
        }
        $('#mediaElements').append($('#tmplMediaEles').html().format(media.duration, src, type, media.name, media.id));
    }
...
function switchRect(obj) {
    if($(obj).parent().parent('ul').length > 0) {
        ...
        var currIndex = $(obj).parent().parent().children('li').index($(obj).parent());
        var parentLi = $(obj).parent().parent().parent();
        var parentIndex = $(parentLi).parent().children('li').index($(parentLi));
        if(drawInfo.operRect) drawInfo.operRect.isSelected = false;
        select(relativePlaylist.children[viewIndex].children[parentIndex].children[currIndex]);
    }

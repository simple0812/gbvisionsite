...
ScheduleView = Backbone.View.extend( {
    ...
    render: function() {
        ...
        var temp = _.template($('#scheduleTemplate').html(),
            {id: data.id, name: data.name, children: data.playlistName || '<font color="red"> 已删除</font>',title: data.playlistName || '', scheduleType: scheduleType,
                scheduleDetail:scheduleDetail,stamp: data.stamp});
        $(this.el).html(temp);

        if(!data.playlistName) this.$el.find('.publish').hide();
        return this;
    },
...
//            else if(name.getRealLength() > 40 ) return  popBy('#renameInput',false,'排期名长度不能超过40字节');

...
//        var that = this;
        var type = $('.scheduleType:checked').val();
        var data = this.model.toJSON();
        var name = $('#name').val().trim();
        if(name === '') return  popBy('#name',false,'排期名不能为空');
//        else if(name.getRealLength() > 40 ) return  popBy('#name',false,'排期名长度不能超过40字节');
        ...
        data.schedule = {};
//        var tmpPlaylist = playlists.get(that.$('#children').val());
//        data.children.push(tmpPlaylist);
        switch (type) {
            case 'once':
            ...
            case 'everymonth':
                ...
                var that = this;
                $.each(tmpMonthDays, function(i, o) {
//                    that.model.toJSON().schedule.day.push(o.toJSON().id);
                    data.schedule.day.push(o.toJSON().id);
                })
                data.schedule.timeFrom = $('#startTime').val();
                data.schedule.timeTo = $('#endTime').val();
                break;
            default :alert('进入default了save');break;
        }
...
//        $('#returnComfirmBtn').click(function() {
//            location.href = '/schedule';
//        });
//    },
